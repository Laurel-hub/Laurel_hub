# Laurel_hub
👋 Hi, I’m Oghenevurie Lauretta  

📊 AI & Data Scientist | MSc Artificial Intelligence  
🏥 Healthcare Background | Research & Applied Analytics  

I work at the intersection of **data analysis, machine learning, and healthcare**, with experience applying statistical and AI methods to real-world problems. My background combines frontline healthcare insight with strong analytical and technical training in AI, data science, and knowledge-based systems.

🔍 **Interests**
- Predictive modelling & statistical learning  
- Machine learning & applied AI  
- Health data analytics & evidence-based research  
- Ontology engineering & knowledge representation  
- Responsible and ethical AI  

🚀 **Currently**
- Building applied AI and data projects across healthcare, analytics, and decision-support systems  
- Developing a curated AI & data portfolio alongside academic and internship work  

🤝 **Open to**
- Entry-level and junior roles in data, health-tech, and AI  
- Research assistant and analyst opportunities  
- Collaboration on meaningful, impact-driven data projects  

🌱 **Outside of work**
I enjoy reflective writing, spoken-word poetry, deep conversations, and lifelong learning.

🔗 **Connect with me**
- See LinkedIn: www.linkedin.com/in/lauretta-oghenevurie-063a772a4   
